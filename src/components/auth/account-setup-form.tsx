...
export default function AccountSetupForm() {
	...
	const { mutate: accountSetup, isPending } = useMutation({
		mutationFn: (data: SetupRequest) => auth.setup(data),
		onSuccess: () => {
			navigate({
				to: '/'
			})
			toast.success(`Welcome to threads ${user?.username} !`)
		},
										...
										<div className='w-full'>
											...
											<div className=' flex items-center gap-2  w-full my-1 h-7'>
												...
												<div className='flex-grow overflow-hidden outline-none text-[15px] text-accent-foreground break-words tracking-wide w-full select-none'>
													{`${user?.fullname ?? ''} ${'(' + userAccountData?.username + ')'}`}
												</div>
											</div>
										</div>

...
const CreatePostInput: React.FC<CreatePostInputProps> = ({ isOpen, replyThreadInfo, onTextareaChange, quoteInfo }) => {
	const { user } = useUser()
		...
		>
			<div className='relative flex flex-col items-center'>
				{replyThreadInfo ? (
					...
					<UserAvatar image={user?.image} username={user?.username ?? ''} fullname={user?.fullname} />
				)}
			...
			<div className='flex flex-col w-full gap-1.5 pb-4'>
				{replyThreadInfo ? (
					...
					<span className='text-[15px] font-medium leading-none tracking-normal'>{user?.username}</span>
				)}
				...
				{quoteInfo && <PostQuoteCard {...quoteInfo} createdAt={quoteInfo.createdAt} />}
			</div>
		</div>
	)
}

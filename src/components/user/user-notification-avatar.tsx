...
const UserNotificationAvtar: React.FC<UserNotificationAvtarProps> = ({ username, image, fullname, type }) => {
		...
		<Link href={`/@${username}`}>
			<div className='outline outline-1 outline-border rounded-full ml-[1px]'>
				<Avatar className='h-10 w-10 relative overflow-visible cursor-pointer '>
					...
					<div
						className={cn('absolute -bottom-1 -right-1 rounded-2xl border-2 border-background text-white', {
							'bg-[#fe0169]': type === 'LIKE',
